...
public partial class Meetups : LNPage
{
    ...
    private string DibujaCard(Meetup _meet, string strClima, float temp)
    {
        ...
        int cantCervezas = 0;
        bool registrado = _meet.LPer.Contains(UserInfo.Instance.user.PerID);
        bool perCheckin = (_meet.LPerConfirm.Contains(UserInfo.Instance.user.PerID) ? true : false);
        bool Finalizado = (_meet.MeetFecha < DateTime.Now ? true : false);

        sb.AppendLine("<div class='col-sm-6 col-lg-3 " + (registrado ? "Registrado" : "SinRegistrar") + " " + (Finalizado ? "Finalizado" : "Proximo") + "'>");
    ...
    private async void GetMeetups()
    {
        ...
        try
        {
            ...
            using (var response = await client.SendAsync(request))
            {
                ...
                divCards.InnerHtml = sb.ToString();
            }
        }
        catch (Exception ex)
        {
    ...
    protected async void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            btnNuevo.Visible = UserInfo.Instance.user.AdminUser;
